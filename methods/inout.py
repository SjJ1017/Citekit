...
if __name__ == '__main__':
    ...
    llm_inst = demo['instruction'] + ' When citing documents, prioritize the use of external documents, and if the external documents can not entail your answer, then try to use inner passages.'
    llm_inst = llm_inst.replace('provided search results (some of which might be irrelevant)','provided external documents and inner passages (some of which might be irrelevant)')
    print(llm_inst)
    prompt = Prompt(template='<passage><shots><question><RP>',components= {'passage':'{passage}\n\n', 'shots':'{shots}\n','question':'Question:{question}\n\n','RP':'\n{RP}\nAnswer:'})
    llm_prompt = Prompt(template='<shots><INST><question><docs><passage><answer>',components= {'INST':'{INST}\n\n','docs':'External documents:\n{docs}','passage':'\nInner passages:\n{passage}\n\nAnswer:\n','question':'Question:{question}\n\n','shots':'{shots}\n','answer':'\nAnswer: {answer}\n'})
    ...
    llm_shots = '\n'.join(llm_prompt.load_data(demo['demos'][:2],'question','answer', 
                                               INST = lambda data: llm_inst,
                                               docs = lambda data: ''.join(ALCEDocPrompt().default_load_data(data['docs'][:args.ndoc])),
                                               passage = lambda data: ''.join(ALCEDocPrompt().default_load_data(data['docs'][args.ndoc:2*args.ndoc],from_idx=args.ndoc))))
    dataset = PromptDataset(dataset,'question', 'answer', 'answers' ,'qa_pairs','claims', docs = lambda data: ALCEDocPrompt().default_load_data(data['docs'][:args.ndoc]))[:args.data_num]
    
    # PIPELINE
    eval = DefaultEvaluator(args)
    recite = R_LLM(model=args.recite_model,prompt_maker=prompt, self_prompt={'shots':recite_shots,'RP':RECITE_PROMPT},noisy=False)
    ...
    # RUN PIPELINE
    pipeline.run_on_dataset(datakeys=['question', 'docs'],initial_module=recite, init_docs='docs')